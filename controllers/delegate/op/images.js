...
import confirmFrameSvg from '../../../utils/delegate/op/confirmFrameSvg.js';
import delegateFrameSvg from '../../../utils/delegate/op/delegateFrameSvg.js';
import noDelegateFrameSvg from '../../../utils/delegate/op/noDelegateFrameSvg.js';
import shareMyDelegateFrameSvg from '../../../utils/delegate/op/shareMyDelegateFrameSvg.js';
import successFrameSvg from '../../../utils/delegate/op/successFrameSvg.js';

export async function delegateOpConfirm(request, reply) {
...
export async function delegateOpDelegate(request, reply) {
  const timestamp = Math.floor(new Date().getTime() / 1000);
...
export async function delegateOpNoDelegate(request, reply) {
  const timestamp = Math.floor(new Date().getTime() / 1000);
  let user = request.query.user;
  let balance = request.query.balance;
  let userShortAddress = request.query.ushort;
...
export async function delegateOpShare(request, reply) {
  const timestamp = Math.floor(new Date().getTime() / 1000);
  let user = request.query.user;
  let delegate = request.query.delegate;
  let balance = request.query.balance;
...
export async function delegateOpSuccess(request, reply) {
  const timestamp = Math.floor(new Date().getTime() / 1000);
